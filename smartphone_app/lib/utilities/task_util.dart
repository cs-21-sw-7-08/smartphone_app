...
class TaskUtil {
  static Future<T?> _syncNullable<T>(T? Function() function) {
  ...
  static Future<Return?> runTask<T, Return>(
      {required BuildContext buildContext,
      required String progressMessage,
      required Future<T?> Function(bool Function() runTask) doInBackground,
      required Return? Function(T? value) afterBackground,
      required Function() taskCancelled}) async {
    var runTask = true;
    CancelableOperation<Return?>? cancelableOperation;
    ...
    Future task = Future.delayed(const Duration(milliseconds: 500), () async {
      ...
      // Operation
      cancelableOperation = CancelableOperation.fromFuture(_syncNullable(() {
        if (!runTask) return null;
        return afterBackground(response);
      }), onCancel: () => null);
      // Wait for value
